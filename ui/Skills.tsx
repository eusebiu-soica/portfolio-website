...
export default function Skills() {
    ...
    const getRandomDelay = () => Math.random() * 1;

    return (
        <section data-section='3' className="snap-section min-h-max flex flex-col gap-16 relative items-start overflow-hidden px-8 lg:px-28 py-24 bg-background">
            <div className="z-10">
                <h1 className="text-3xl lg:text-6xl font-extrabold">
                    <span className="text-primary">Tech</span> Arsenal
                </h1>
            </div>
            <Lottie animationData={Tech} className="absolute lg:rotate-90 bottom-0 -top-20 right-0 left-0  z-0 opacity-50"/>
                ...
                {skills.map((skill) => (
                    <motion.div
                        ...
                        whileHover={{
                            scale: 1.1,
                            rotate: 2,
                            transition: { type: "bounce", stiffness: 300, damping: 10, duration:0.1 },
                        }}
                        className={clsx(
                    ...
                    >
                        <div className="z-10 flex flex-col items-center gap-4">
                           {skill.icon && (<div className="text-3xl text-primary"><Image draggable={false} src={skill.icon} alt={skill.name} className="w-8 h-8"/></div>)}
                            <span className={clsx("text-white font-medium text-sm text-center")}>{skill.name}</span>
                        </div>
                    </motion.div>
                ))}
            </div>
